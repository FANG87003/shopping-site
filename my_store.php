...
// 獲取使用者的所有商品
$sql_products = "SELECT * FROM `products` WHERE `owner` = ? ORDER BY `id` DESC";
$stmt_products = $db_link->prepare($sql_products);
if (!$stmt_products) {
    die("商品查詢準備語句錯誤: " . $db_link->error);
}
$stmt_products->bind_param("s", $user_id);
...
<head>
    ...
    <style>
        ...
        .add-button {
            /* 移除 display: block; 和 margin: auto; */
            padding: 12px 20px;
                ...
                <?php foreach ($products as $product): ?>
                    ...
                    <div class="product-card">
                        ...
                        <div class="product-info">
                            ...
                            <p><strong>分類：</strong>
                                <?= htmlspecialchars($product['category_ids'], ENT_QUOTES, 'UTF-8'); ?>
                            </p>
                            <p><strong>標籤：</strong>
                                <?php
                        ...
                        <div class="product-actions">
                            <a class="action-button" href="edit_product.php?id=<?= htmlspecialchars($product['id']); ?>">編輯</a>
                            <a class="action-button" href="delete_product.php?id=<?= htmlspecialchars($product['id']); ?>" onclick="return confirm('確定要刪除此商品嗎？');">刪除</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
